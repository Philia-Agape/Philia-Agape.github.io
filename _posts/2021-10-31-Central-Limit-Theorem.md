...
The alternative way of central limit theorem is, Given $$ X_{i} \sim \mathcal{N}(\mu,\sigma)$$, let new random variable $$ Y_{i} = \frac{X_{i}-\mu}{\sigma}$$, then $$\mathbb{E}(Y) = \frac{\mathbb{E}(X_{i})-\mu}{\sigma} = 0$$, and $$ Var(Y) = \mathbb{E}(Y^2) - \mathbb{E^2}(Y) = \mathbb{E}(\frac{X^2_{i} - 2\muX_{i} + \mu^2}{\sigma^2}) = \frac{\mu^2+\sigma^2-2\mu^2+\mu^2}{\sigma^2} = 1$$; so $$Y_{i} \sim \mathcal{N}(0,1)$$, finally define the sample random variable $$Z  = \frac{\sum_{i=1}^{N} Y_{i}}{N}$$, we may conclude that 
